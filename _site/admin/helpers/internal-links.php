...
function autoLinkInternalArticlesInHtml(string $html, array $options = []): array {
    ...
    if ($wordCount < $minWords) {
        libxml_clear_errors();
        libxml_use_internal_errors($prev);
        return ['html' => normalizeBodyHtml($dom, $body), 'added' => 0, 'skipped_short' => true];
    }
    ...
    if ($articleOptions === []) {
        libxml_clear_errors();
        libxml_use_internal_errors($prev);
        return ['html' => normalizeBodyHtml($dom, $body), 'added' => 0, 'skipped_short' => false];
    }
    ...
    return [
        'html' => normalizeBodyHtml($dom, $body),
        'added' => $added,
        'skipped_short' => false,
    ];
}
