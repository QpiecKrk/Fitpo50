(() => {
  ...
  type FitWindow = Window & {
    FitPo50Consent?: ConsentState;
    gtag?: (...args: unknown[]) => void;
    __fitpo50CmpPatched?: boolean;
    __fitpo50AnalyticsInjected?: boolean;
    __fitpo50GtagBootstrapped?: boolean;
  };

  const win = window as FitWindow;
  ...
  const hasAnalyticsConsent = () => Boolean(consentState?.analytics);
  const hasMarketingConsent = () => Boolean(consentState?.marketing);
  ...
  const panel = banner.querySelector<HTMLElement>('[data-cmp-panel]');
  const settingsBtn = banner.querySelector<HTMLButtonElement>('[data-cmp-action="toggle-settings"]');
  const analyticsInput = banner.querySelector<HTMLInputElement>('[data-cmp-consent="analytics"]');
  const marketingInput = banner.querySelector<HTMLInputElement>('[data-cmp-consent="marketing"]');

  const saveState = (analytics: boolean, marketing: boolean) => {
  ...
  banner.addEventListener('click', (event) => {
    ...
    const action = target?.closest<HTMLButtonElement>('[data-cmp-action]')?.dataset.cmpAction;
    if (!action) return;
    ...
    if (action === 'save-selected') {
      saveState(Boolean(analyticsInput?.checked), Boolean(marketingInput?.checked));
    }
  });
