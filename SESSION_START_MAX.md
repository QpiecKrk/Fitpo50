...
- publikacja nowego artykulu jest transakcyjna: HTML, listingi, sitemap, `_site`, assety i PDF najpierw powstaja w izolowanym stagingu poza repo. Bezposredni zapis importera jest zabroniony; repo zmienia sie dopiero po statusie `PREVIEW_READY`, a konflikt z rownolegla zmiana blokuje promocje,
- publikacja rozroznia `CREATE` i `UPDATE`; aktualizacja istniejacego slugu wymaga jawnego `--force true`,
- transakcja obejmuje artykul, media, PDF, `index.html`, `porady.html`, kategorie, sitemap, `llms.txt`, `llms-full.txt`, indeks wyszukiwarki, raporty publikacji i odpowiadajace pliki `_site`,
- backup wszystkich nadpisywanych plikow pozostaje aktywny do konca walidacji po promocji; blad cofa caly zestaw, a przerwana transakcja jest odzyskiwana przed nastepna publikacja,
- udana publikacja zapisuje manifest `data/reports/article-publications/<slug>.json` z trybem `CREATE`/`UPDATE`, lista plikow i hashami przed/po; półgotowa publikacja jest twardym FAIL,
- przed `PREVIEW_READY` obowiazkowo renderuj pelny desktop 1440 px i mobile 390 px, wymus zaladowanie lazy images i finalny stan animacji, sprawdz overflow, fonty, proporcje i uszkodzone media,
- tabele nowych artykulow musza byc prawdziwym HTML: `.article-table-wrap`, `caption`, `thead`, `tbody`, `th scope="col"` i w razie naglowkow wierszy `th scope="row"`; obraz tabeli albo awaryjny zapis komorek kreskami jest FAIL,
- PDF generuj ze stagingowego HTML, renderuj wszystkie strony do PNG i kontroluj A4, marginesy/uciecia, osadzone fonty Unicode, komplet ilustracji, tabele oraz zgodnosc tresci HTML→PDF minimum 98%. HTML i PDF w source oraz `_site` musza byc identyczne 1:1,
...
- centra sterowania automatyzacja:
  ...
  - Growth SEO/AEO/GEO/AIO: `npm run growth:<report|audit-ai|gsc-refresh|gsc-generative-ai|post-deploy-kpi|originality-score|evidence-plan|hubs|link-assets|autopilot|verify>`,
  ...
  - raport ma porownywac 7/28/90 dni, sprawdzac dostepne Web/Images/Video/News/Discover/Google News oraz uruchamiac URL Inspection dla artykulow bez wyswietlen,
  - po kazdym uruchomieniu `popraw-seo` obowiazkowo wygeneruj szerokie wnioski dla calej witryny w `popraw-seo-wnioski-globalne.md` i `.json`: udzial widocznych artykulow, stan indeksacji, zindeksowane bez wyswietlen, CTR TOP 10, pozycje 11-30/31+, koncentracja klikniec, klastry/kategorie, trend 28 dni i Google Images,
  - wnioski globalne maja konczyc sie kolejnoscia dzialan oparta na aktualnych liczbach; zmiana priorytetu wymaga zmiany danych, nie opinii automatu,
  - `popraw-seo` ma laczyc w jednej paczce `BOOST` (URL-e blisko wzrostu w GSC: CTR/title/meta/lead/linkowanie), `ROKUJE` (drugi raport: strony poza liderami, ale z sygnalem GSC/strategicznym na kolejna fale wzrostu) oraz `NAPRAWA` (slabe albo niewidoczne strony: quick answer/FAQ/zrodla/linkowanie),
  - po poprawkach liderow agent zawsze sprawdza `ROKUJE`; te strony tez poprawia konkretnie, bez generycznych blokow, i po wdrozeniu podaje URL-e do zgloszenia w GSC,
  ...
  - od 2026-07-21 `popraw-seo` musi pokazywac `GSC Generative AI`, `KPI Po Wdrożeniu` i `Originality / Własne Dane`; brak eksportu Search Generative AI w `~/Downloads/gsc-auto-input` oznacza status `NO_EXPORT_OR_ROLLOUT_NOT_AVAILABLE`, a nie blad,
  - `post-deploy-kpi` ma mierzyc 7/14/28 dni szerzej niz CTR: Web GSC, AI impressions/clicks, engaged time, CTA/PDF clicks, przejscia do centrów i powroty,
  - `originality-score` jest bramka jakosci: flaga braku wlasnego przykladu FitPo50, liczby/progu, tabeli/PDF/grafiki, zrodel przy claimach albo warunku bezpieczenstwa; nie wolno automatycznie dopisywac ogolnikow, trzeba przygotowac konkretny fragment albo oznaczyc `INSUFFICIENT_DATA`,
...
- skrot `GSC` w rozmowie oznacza domyslnie:
  ...
  - przed raportem obowiązkowo przejdź `npm run gsc:data:check -- --input-dir ~/Downloads/gsc-auto-input`; wymagany jest `gsc-data-manifest.json`, wiek danych maks. 72 h, końcowa data zakresu maks. 3 dni wstecz, wspólny cohort plików maks. 30 min, poprawne typy nagłówków, zgodne SHA-256/liczby wierszy i ciągłe okna 7/28/90,
  - `pages.csv` skopiowane jako `query-pages.csv`, rozbieżne aliasy `query-pages.csv`/`query_pages.csv`, stare property API albo luźne CSV bez manifestu są błędem blokującym; nie generuj na nich `popraw-seo`,
  - limit/paginacja API musi być jawna w `collection_quality`; osiągnięcie 50 000 wierszy oznacza `WARN_LIMIT_REACHED`, a nie przemilczaną kompletność query,
  - inventory raportu rozdziela zwykłe artykuły redakcyjne i centra tematyczne mające `BlogPosting`, ale kontrakt pełnego pokrycia obejmuje obie grupy i nadal wymaga `omitted_articles = 0`,
